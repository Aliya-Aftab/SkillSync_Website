...
import { FiLoader, FiSearch, FiRefreshCw, FiCheck, FiX } from "react-icons/fi";
...
const Feed = () => {
  ...
  // 2. The Trigger Function (Passed to Child)
    ...
    <div className="flex flex-col items-center min-h-screen pt-28 pb-20 relative overflow-hidden">
      ...
      <div className="relative z-10 w-full flex flex-col items-center">
        ...
        <div className="w-full max-w-[380px] drop-shadow-2xl animate-fade-in-up px-4 relative">
           ...
           {/* 3. Pass the Trigger to UserCard */}
      ...
      {toast.show && (
        <div className="fixed bottom-10 right-10 z-50 animate-fade-in-up">
          <div className="bg-slate-900 text-white px-6 py-4 rounded-xl shadow-2xl flex items-center gap-3 border border-slate-700">
            <div className={`w-6 h-6 rounded-full flex items-center justify-center text-slate-900 ${toast.type === 'error' ? 'bg-rose-500' : 'bg-green-500'}`}>
               {toast.type === 'error' ? <FiX className="text-sm" /> : <FiCheck className="text-sm" />}
            </div>
            <div>
